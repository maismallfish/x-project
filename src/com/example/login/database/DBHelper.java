...
public class DBHelper {
	 ...
	 */
	public long insertOrUpdate(String userName, String usrId, String password, int isSaved) {
		...
		long id = -1;
		if (isUpdate) {
			id = update(userName, usrId, password, isSaved);
		} else {
			if (db != null) {
	 ...
	 */
	public long update(String username, String userId, String password, int isSaved) {
		ContentValues values = new ContentValues();
		values.put(User.USERNAME, username);
		values.put(User.PASSWORD, password);
		values.put(User.ISSAVED, isSaved);
